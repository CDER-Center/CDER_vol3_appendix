...
// SSSP V5 - 3 push shmem // STILL SLOWER THAN V4 -- TESTING -rajz
__global__ void csrKernelBellmanFordMoore3(int N, int M, const int *d_nindex, const int* d_nlist, const int* d_eweight,
  ...
  int *parent // not req
    ...
    __shared__ int shmem_nlist[MAX_INT_IN_SHARED_PER_BLOCK];
    ...
    for (i = start, j = 0, minSize = (size < SH_REGS_PER_THREAD ? size : SH_REGS_PER_THREAD); i < end && j < minSize; ++i, ++j)
      shmem_nlist[threadIdx.x * SH_REGS_PER_THREAD + j] = d_nlist[i];
		...
		for( i=start, j =0; i< end; i++, ++j){ 
			int v ; //= d_nlist[i]; // Instead of a read from GMEM
      if(j < SH_REGS_PER_THREAD)
        v = shmem_nlist[threadIdx.x * SH_REGS_PER_THREAD + j];  // If in ShMem take it
      else
        v = d_nlist[i];  // Else Read from Global
			/// relax
			int newDist =  minDist[u]+ d_eweight[i];
			if(newDist < minDist[v]){ //To avoid conjection of atomic stmts
				atomicMin(&minDist[v], newDist); 
...
static int* gpuSSSP(const ECLgraph& g){
  ...
  cudaDeviceSynchronize();
  const double runtime = timer.stop();
  printf("FPT GPU SSSP. DEVICE: %12.9f s\n", runtime);
...
int main(int argc, char *argv[]){
  ...
  //printf("GPU: %s with %d SMs and %d mTpSM (%.1f MHz and %.1f MHz)\n\n", deviceProp.name, SMs, mTpSM, deviceProp.clockRate * 0.001, deviceProp.memoryClockRate * 0.001);  fflush(stdout); //clockRate removed in CUDA 13
  printf("GPU: %s with %d SMs and %d mTpSM \n\n", deviceProp.name, SMs, mTpSM);  fflush(stdout); 
